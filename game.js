...
function addMessage(text) {
  messages.unshift({ text, timer: 120 });
  if (messages.length > 5) messages.pop();
}

// ===== Pixel world config =====
const TILE = 16;            // 16x16 pixel tiles (classic SNES/GB vibe)
const WORLD_W = 300;        // tiles wide
const WORLD_H = 240;        // tiles high

// Tile IDs
// 0 grass/walkable, 1 tree/solid, 2 water/solid, 3 mountain/solid, 4 hut/solid, 5 cave/solid
...
// ===== Tools =====
const TOOLS = { NONE:0, AXE:1, BOAT:2, PICKAXE:3, SWORD:4 };
function toolName(t) {
  return t === TOOLS.AXE ? "Axe" : t === TOOLS.BOAT ? "Boat" : t === TOOLS.PICKAXE ? "Pickaxe" : t === TOOLS.SWORD ? "Sword" : "None";
}
...
// After spawn we will place items to reachable tiles
...
const player = {
  ...
  facingDy: 1,
  swingTimer: 0,
  inv: { axe:false, boat:false, pickaxe:false, sword:false },
  health: 10,
  maxHealth: 10,
  invulFrames: 0
};
...
function rand(a, b) { return (Math.random() * (b - a + 1) | 0) + a; }
...
function update() {
  if (respawnOverlayActive) {
    ...
    for (let i = heartDrops.length - 1; i >= 0; i--) {
      ...
      d.timer--; if (d.timer <= 0) heartDrops.splice(i,1);
    }
    for (let i = deathDrops.length - 1; i >= 0; i--) {
      const d = deathDrops[i];
      d.timer--; if (d.timer <= 0) relocateDrop(i);
    }
    for (let i = messages.length - 1; i >= 0; i--) {
      messages[i].timer--; if (messages[i].timer <= 0) messages.splice(i,1);
    }
    return;
  }
  ...
  // Interact with target tile when moving into it (chop/mine)
  if (player.isMoving) {
    const nextCx = player.x + vx + player.w/2;
    const nextCy = player.y + vy + player.h/2;
    const tx = Math.floor(nextCx / TILE);
    const ty = Math.floor(nextCy / TILE);
    if (inside(tx, ty)) {
      const t = map[ty][tx];
      if (t === T.TREE && player.tool === TOOLS.AXE) {
        map[ty][tx] = T.FLOOR;
      } else if (t === T.MOUNTAIN && player.tool === TOOLS.PICKAXE) {
        map[ty][tx] = T.FLOOR;
      }
    }
  }
...
function draw() {
  ...
  if (player.tool === TOOLS.BOAT && onWater) {
  ...
  // Sword swing effect
  if (player.swingTimer > 0 && (player.tool === TOOLS.SWORD || player.tool === TOOLS.AXE || player.tool === TOOLS.PICKAXE)) {
    ctx.fillStyle = player.tool === TOOLS.SWORD ? "#e6e6e6" : (player.tool === TOOLS.AXE ? "#d0b48a" : "#bfbfbf");
    const fx = player.facingDx, fy = player.facingDy;
    const cx = player.x + player.w/2;
    const cy = player.y + player.h/2;
    if (fx === 1) {
      ctx.fillRect(cx+4, cy-4, 6, 2);
  ...
  if (respawnOverlayTimer > 0) {
    ...
    ctx.fillText("You fainted!", VIEW_W/2, VIEW_H/2);
    ctx.font = "20px system-ui, sans-serif";
    ctx.fillText("Press Space to respawn", VIEW_W/2, VIEW_H/2 + 30);
    respawnOverlayTimer--;
  }
  ...
  const invStr = `[${player.inv.axe?"A":"-"}${player.inv.boat?"B":"-"}${player.inv.pickaxe?"P":"-"}${player.inv.sword?"S":"-"}]`;
  let hudY = VIEW_H - 10;
  ctx.fillText("WASD/Arrows • 1:Axe 2:Boat 3:Pickaxe 4:Sword • Space: Use • Tool: " + toolName(player.tool) + " • Inv " + invStr, 10, hudY);
  // Pickup messages
  hudY -= 16;
  ...
  if (respawnOverlayActive) {
    ...
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    ctx.fillText("You fainted...", VIEW_W/2, VIEW_H/2 - 10);
    ctx.font = "18px system-ui, sans-serif";
    ctx.fillText("Press Enter or Space to continue", VIEW_W/2, VIEW_H/2 + 20);
  }
}
...
function solidAt(px, py) {
  ...
  const t = map[ty][tx];
  if (t === T.WATER && player.tool === TOOLS.BOAT) return false;
...
function updateSpiders() {
  const aggroDist = 90;   // reduced
  const leashDist = 140;  // reduced
  for (const s of spiders) {
  ...
  for (let i = spiders.length - 1; i >= 0; i--) {
    const sd = spiders[i];
    if (sd.hp <= 0) {
      const amount = sd.type === "large" ? 2 : 1;
...
function applyDamage(amount) {
  ...
  player.invulFrames = 45;
  if (player.health === 0) {
    // drop inventory on death
    dropInventoryAt(player.x + player.w/2, player.y + player.h/2);
    // respawn elsewhere
    const newSpawn = findDifferentSpawn(player.x, player.y);
    player.x = newSpawn.x;
    player.y = newSpawn.y;
...
function dropInventoryAt(cx, cy) {
  ...
  for (const k of kinds) {
    deathDrops.push({ kind: k, x: cx-8, y: cy-8, w: 16, h: 16, timer: 7200 });
  }
}
...
function findDifferentSpawn(prevX, prevY) {
  for (let tries = 0; tries < 5000; tries++) {
    ...
    if (dx*dx + dy*dy > (TILE*TILE*100)) return s;
  }
  return findSpawn();
}
